...
contract NFTMarketwithBid is ReentrancyGuard,Pausable {
...
mapping (uint256 =>BidDetails) public bidtoTokenID;
  ...
  function placebid (address nftContract, uint256 tokenId, uint256 itemId) public payable whenNotPaused {
      require(idToMarketItem[itemId].seller != msg.sender, "Sorry! seller can't place the bid on his own order");
      require(msg.value >= listingPrice && msg.value > bidtoTokenID[tokenId].price, "Keep Bid price greater than last bid on this token");

    MarketItem memory marketItem = _getValidOrder(nftContract, itemId);
    if (nftContract != marketItem.nftContract || tokenId != marketItem.tokenId || itemId != marketItem.itemId){
        revert("Sorry! wrong combination bid not allowed");
    } else if
    (_bidderHasABid(tokenId, msg.sender ) == false ) {
        revert("your Bid is already in place for this token");
    } else  {
      
     _cancelBid( tokenId,  itemId);
       
      bidtoTokenID[tokenId] =  BidDetails(
      itemId,
      nftContract,
      payable(msg.sender),
      msg.value
    ...
    function _cancelBid(uint256 tokenId,  uint256 itemId) internal {
          uint256 cancelledBidprice = bidtoTokenID[tokenId].price;
          address payable cancelledbidder = bidtoTokenID[tokenId].bidder;
    ...
    function cancelBid(uint256 tokenId,  uint256 itemId) public whenNotPaused {
        
        BidDetails memory bid = bidtoTokenID[tokenId];
        require(bid.bidder == msg.sender || msg.sender == owner,"Marketplace: Unauthorized sender to cancel the Bid!");
        _cancelBid( tokenId,  itemId);
        }
         
    function _bidderHasABid(uint256 tokenId, address bidder) internal view returns (bool)    {
         
         BidDetails  memory bidDetails  = bidtoTokenID[tokenId];
   ...
   function acceptBid (address nftContract, uint256 tokenId, uint256 itemId) public nonReentrant whenNotPaused {
        ...
        uint256 price = bidtoTokenID[tokenId].price;
        address payable tokenPurchaser = bidtoTokenID[tokenId].bidder;
      ...
      function removeMarketItem(address nftContract,   uint256 itemId) public nonReentrant whenNotPaused {
