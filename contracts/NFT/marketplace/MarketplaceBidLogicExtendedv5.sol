  ...
  event soldNFT(address indexed nftContract, uint256 tokenId, uint256 itemId, address indexed buyer, address indexed previousOwner,
   uint256 NFTsoldAmount, uint256 buyerReceviedAmount);
    ...
    emit soldNFT (nftContract, tokenId, itemId, msg.sender, previousOwner, price, saleAmount);    
      ...
      function itemsSOldbyMarketplace(address nftContract ) public view returns (MarketItem[] memory) {
    ...
    for (uint i = 0; i < totalItemCount; i++) {
      ...
      idToMarketItem[i + 1].removedFromMarketplace == false && idToMarketItem[i + 1].nftContract == nftContract ) {
        itemCount += 1;
      }
    }
    ...
    return items;
          ...
          /* Returns Every items that available on market  for Sale */
      ...
      function fetchMarketItemsbyNftAddress(address nftContract) public view returns (MarketItem[] memory) {
        ...
        return items;
             ...
             /* Returns only specific items a user has created by nftContract address */
      function fetchItemsCreated(address nftContract) public view returns (MarketItem[] memory) {
        ...
        return items;
          ...
          /* Returns only items that a user has Sold through Marketplace */
      function userNFTsSoldbyThis(address nftContract) public view returns (MarketItem[] memory) {
        uint totalItemCount = _itemIds.current();
        uint itemCount = 0;
        uint currentIndex = 0;
    ...
    /* Returns only items that a user has purchased  --shown by nftContract*/
      function fetchMyNFTsPurchasedThroughMarketplace(address nftContract) public view returns (MarketItem[] memory) {
        uint totalItemCount = _itemIds.current();
        uint itemCount = 0;
        uint currentIndex = 0;
    
        for (uint i = 0; i < totalItemCount; i++) {
          if ( idToMarketItem[i + 1].owner == msg.sender  && idToMarketItem[i + 1].sold == false  && 
          idToMarketItem[i + 1].removedFromMarketplace == false && idToMarketItem[i + 1].nftContract == nftContract) {
            itemCount += 1;
          }
        }
      ...
      /* Returns TokenID's listed on marketplace for any given token*/
      function tokenIDonSale(address nftContract) public view returns (uint256[] memory) {
        uint itemCount = _itemIds.current();
        uint unsoldItemCount = _itemIds.current() - _itemsSold.current();
        uint currentIndex = 0;
    ...
    require(payable(msg.sender).send(address(this).balance));
                 ...
                 /* Returns Bid Details where User Submitted a bid */
